...
vec3 meshGradient(vec2 uv){
  ...
  vec3 c0=vec3(0.05,0.10,0.26); vec3 c1=vec3(0.08,0.16,0.36);
  vec3 c2=vec3(0.03,0.09,0.22); vec3 c3=vec3(0.10,0.20,0.40);
  float e=2.0;
...
function ShaderCanvas({ fragSource, pixelRatio }: { fragSource: string; pixelRatio?: number }) {
  ...
  useEffect(() => {
    ...
    if (!vs) { drawError(gl, vsLog); return cleanup; }
    const { shader: fs, log: fsLog } = safeCompile(gl, gl.FRAGMENT_SHADER, fragSource);
    if (!fs) { drawError(gl, fsLog); gl.deleteShader(vs); return cleanup; }
    const linked = safeLink(gl, vs, fs);
    gl.deleteShader(vs); gl.deleteShader(fs);
    if (!linked.program) { drawError(gl, linked.log); return cleanup; }
    ...
    function tick(now: number) {
      ...
      try {
        ...
        const w = canvas.width, h = canvas.height, dpr = getDpr();
      ...
      } catch(err) { drawError(gl, String(err)); }
...
export default function ShaderBackground() {
  return (
    <div style={{ position: "fixed", inset: 0, width: "100vw", height: "100dvh", background: "black", overflow: "hidden", zIndex: 0 }}>
